...
class DataObject extends Contract {
  ...
  static states = ['INIT', 'REQUESTED', 'ACCEPTED', 'REJECTED', 'CANCELED'];
  ...
  static async readJSONfromState(ctx, key) {
    ...
    if (!object || object.length === 0) {
      throw new Error(`Key ${key} does not exist for ORDER`);
    }
  ...
  async set(ctx, choreographyId, newState) {
    if (!DataObject.states.includes(newState)) {
      throw new Error(`Cannot set state of ORDER to ${newState} (choreography ${choreographyId})`);
    }
    if (!DataObject.stateExists(ctx, choreographyId)) {
      throw new Error(`Cannot set state of ORDER as it does not exist (choreography ${choreographyId})`);
    }
