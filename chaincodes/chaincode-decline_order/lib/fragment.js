...
// Chaincode name: ChoreographyTask_1f6t41p
...
${error}`);
  ...
  static async readJSONfromState(ctx, key) {
    ...
    if (!object || object.length === 0) {
      throw new Error(`Key ${key} does not exist for fragment ChoreographyTask_1f6t41p`);
    }
  ...
  async getState(ctx, choreographyId) {
    ...
    return {
      sequenceFlowState,
      taskEnablement: {
        ChoreographyTask_1f6t41p: (await Fragment.isChoreographyTask_1f6t41pEnabled(ctx, choreographyId)).enabled
      }
    };
  }
...
// Precondition for task: 'Decline order'
static async isChoreographyTask_1f6t41pEnabled(ctx, choreographyId) {
  try {
    // Check if fragment is active
...
// Task: 'Decline order'
async ChoreographyTask_1f6t41p(ctx, choreographyId) {
  const taskEnablement = await Fragment.isChoreographyTask_1f6t41pEnabled(ctx, choreographyId);
  ...
  const modifiedDataObjects = [
    JSON.parse('{\"name\":\"ORDER\",\"states\":[\"REJECTED\"]}')
  ];
