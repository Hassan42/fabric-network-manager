...
// Chaincode name: ChoreographyTask_0qrbrpb
...
${error}`);
  ...
  static async readJSONfromState(ctx, key) {
    ...
    if (!object || object.length === 0) {
      throw new Error(`Key ${key} does not exist for fragment ChoreographyTask_0qrbrpb`);
    }
  ...
  async getState(ctx, choreographyId) {
    ...
    return {
      sequenceFlowState,
      taskEnablement: {
        ChoreographyTask_0qrbrpb: (await Fragment.isChoreographyTask_0qrbrpbEnabled(ctx, choreographyId)).enabled
      }
    };
  }
...
// Precondition for task: 'Cancel order'
static async isChoreographyTask_0qrbrpbEnabled(ctx, choreographyId) {
  try {
    // Check if fragment is active
...
// Task: 'Cancel order'
async ChoreographyTask_0qrbrpb(ctx, choreographyId) {
  const taskEnablement = await Fragment.isChoreographyTask_0qrbrpbEnabled(ctx, choreographyId);
  ...
  const modifiedDataObjects = [
    JSON.parse('{\"name\":\"ORDER\",\"states\":[\"CANCELED\"]}')
  ];
